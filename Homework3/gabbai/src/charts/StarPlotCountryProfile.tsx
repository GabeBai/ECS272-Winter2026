...
export function StarPlotCountryProfile({
  data,
  selectedCountry,
  onSelectCountry
}: {
  ...
  const topList = useMemo(() => data.slice(0, 20), [data]);
  ...
  useEffect(() => {
    if (selectedCountry) setSelectedCode(selectedCountry);
  }, [selectedCountry]);

  const selected = useMemo(() => {
    const code = selectedCountry ?? selectedCode;
    return data.find((d) => d.country_code === code) ?? data[0];
  }, [data, selectedCode, selectedCountry]);
